...
<div id="grid1" class="nui-datagrid" style="width:99.5%;height:auto;" sortMode="client"
	...
	<div property="columns">
		...
		<div field="RECEIPT_BALANCE" headerAlign="center" allowSort="true" dataType="currency">贷款余额</div>
 		<div field="END_DATE" headerAlign="center" allowSort="true" dateFormat="yyyy-MM-dd" >首次检查日期</div>
...
<script type="text/javascript">
	...
	grid.on("preload",function(e){
   		...
   		for (var i=0; i<e.data.length; i++){
   			if(e.data[i]['TEAM_USER_NUM'] == userId){
				//如果管户经理是当前登录人 则可以操作
				e.data[i]['op']='<a href="#" onclick="toGo(\''+e.data[i].RECEIPT_NUM+'\');">跳转至贷后检查</a>';
			}
	...
	var url = "/aft/firstCheck/firstCheck_enter.jsp?receiptNum="+receiptNum;
	if('2'=='<%=orgdegree%>'){
		url="/aft/firstCheck/firstCheck_small_enter.jsp?receiptNum="+receiptNum;
	}
	nui.open({
            url: nui.context+url,
            title: "授信后首次检查", 
