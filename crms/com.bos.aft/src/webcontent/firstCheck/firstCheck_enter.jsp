...
<center>
<fieldset>
<legend> <span>贷后首次检查借据列表</span> </legend>
	<div id="form" style="border-top:1px solid #8ba0bc;border-bottom:none;text-align:left;margin-top:7px"  class="nui-form">
		...
		<div id="grid" class="nui-datagrid" sortMode="client"
		    url="com.bos.aft.firstCheck.findBorList.biz.ext" dataField="items"
		    ...
		    <div property="columns">
				...
				<div field="FINISH_DES" headerAlign="center" align="center">完成情况</div>
		    </div>
		</div>
...
<script type="text/javascript">
	...
	function add(){
		...
		if(row.CHECK_STATUS == "03") {
			return nui.alert("该借据已做过首次检查！");
		}
		
		nui.get("btnCreate").setEnabled(false);
		var reportName = row.PARTY_NAME+" "+row.SUMMARY_AMT+"元 "+row.PRODUCT_NAME+" 授信后首次检查";
		var json=nui.encode({"summaryId":row.SUMMARY_ID,"partyId":row.PARTY_ID,"contractId":row.CONTRACT_ID,"isSmall":"0","reportName":reportName});
		$.ajax({
	        ...
	        success: function (data) {
            	...
            	//git.go(nui.context+"/crt/con_info/con_tree.jsp?contractId="+data.tbConContractInfo.contractId+"&contractType=02&amountDetailId="+row.AMOUNT_DETAIL_ID,parent);
	           	nui.open({//传值到tree页面
		            url: nui.context+"/aft/firstCheck/firstCheck_tree.jsp?firstCheckId="+data.tbAftFirstCheck.firstCheckId+"&partyId="+row.PARTY_ID+"&isSmall=0"+"&processInstId="+data.processInstId+"&proFlag=1",
		            showMaxButton: false,
		            title: "首次检查",
		            width: 1024,
		            height: 768,
