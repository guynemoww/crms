 ...
 function onShowRowDetail1(e) {
        ...
        */ 
		if (window.datas[row.dictid]) {
			...
			editTable.setData(fatherWarnInfos[dictid]);//editTable.load({dicttypeid:dicttypeid,"dictid":dictid+"____",partyId:corpid});
...
/*初始化方法*/
 function initForm() {
                    ...
                    if(review==1){
                        ...
                        grid.load({partyId:corpid,status:7,bizId:bizId});
                    }
...
function save(){
    ...
    var json=nui.encode({"corpid":"<%=request.getParameter("corpid") %>","ob":productArray,"status":7,bizId:bizId});   //6表示总行认定岗新增信号状态
    $.ajax({                                                         
...
//刷新頁面
function reload(){
var url=nui.context+"/ews/warnDetail/warnMgr/ews_warnInfo_add.jsp?corpid=<%=request.getParameter("corpid") %>";
git.go(url);
}
