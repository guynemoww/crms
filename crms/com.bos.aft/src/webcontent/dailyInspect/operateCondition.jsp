...
<body>
	...
	<input name="pageName" id="pageName" class="nui-hidden" value="operateConditionPage"/>
...
<script type="text/javascript">
	...
	function reLoad(){
   	 var url;
	 if(goEdit=="1"){
	   url=nui.context+"/aft/dailyInspect/operateCondition.jsp?param="+nui.encode(param)+"&callback="+callback+"&getFirst=2";
	 }else{
	   url=nui.context+"/aft/dailyInspect/operateCondition.jsp?param="+nui.encode(param)+"&callback="+callback+"&getFirst=1";
	 }
	 git.go(url);
	}
	...
	function initGrid() {
		...
		for(var j=1;j<=k;j++) {//构造k个选项卡
		   ...
		   /**填写项目资本金等
		    ...
		    //指标数据ID
		    if(indexResults[h].data1s == null||indexResults[h].data1s[0] == null
		     ||indexResults[h].data1s[0].aliDataId==null||typeof(indexResults[h].data1s[0].aliDataId)=="undefined") {
		    	v_aliDataId = null;
		    }else{
