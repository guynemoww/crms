...
<script type="text/javascript">
	nui.parse();
	var bizId = "<%=request.getParameter("bizId") %>"; // ;"<%=request.getParameter("corpid") %>"业务ID
	var party;                                                   //参与人实体 
	var adjust;                                                  //预警变更实体
	var menudata;
	function init(){
	var json = nui.encode({bizId:bizId});
	 $.ajax({
            ...
            success: function (text) {
            	...
            	if(adjust.changeType==01){
            	 menudata = [
		                         {id:"客户预警", text:"客户预警", //expanded:true, 
			                          children:[
				                                {id:"预警信号列表",text:"预警信号列表",url:"/ews/warnDetail/warnInfo/ews_warning_list.jsp?bizId="+bizId},
				                                {id:"新增预警信号审核",text:"新增预警信号审核",url:"/ews/warnDetail/warnMgr/ews_warnInfo_addView.jsp?corpid="+party.partyId+"&bizId="+bizId+"&review=1"}
			                                    ]}
		                       ];  
            	}else{
                 menudata = [
		                         {id:"客户预警", text:"客户预警", //expanded:true, 
			                          children:[
				                                {id:"预警信号列表",text:"预警信号列表",url:"/ews/warnDetail/warnInfo/ews_warning_list.jsp?corpid="+party.partyId},
			                                    {id:"预警级别认定审核",text:"预警级别认定审核",url:"/ews/warnDetail/warnMgr/review/ews_warnLevel_hold.jsp?bizId="+bizId+"&partyId="+party.partyId}
			                                    ]}
		                       ]; 
