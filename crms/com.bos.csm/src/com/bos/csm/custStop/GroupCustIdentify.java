 ...
 * 集团认定流程
 * */
public class GroupCustIdentify implements IBusiness {
	...
	public void updateBizDataWhenStopFlow(String processInstId, String bizId)
		...
		DataObject group = DataObjectUtil.createDataObject("com.bos.dataset.csm.TbCsmGroupCompany");
...
//			获取流程中的业务数据
			List<Object> list = RelativeDataManagerService.getRelativeDataBatch(Long.valueOf(processInstId), xpath);
				...
				group.set("partyId", list.get(0).toString());
				group.set("status","01");//status=01 表示该集团客户未认定
				DatabaseUtil.updateEntity("default", dataOb);
				DatabaseUtil.updateEntity("default", group);
				//将修改的状态传回流程中
