...
<script type="text/javascript">
	...
	var expireCheckId ="<%=request.getParameter("expireCheckId") %>"; //首次检查主键
	...
	var json = nui.encode({partyId:partyId,checkId:expireCheckId,flag:"1"});
    $.ajax({
	...
	*/
	...
	menudata = [
			{id:"检查信息", text:"检查信息",
				children:[
					{id:"检查信息",text:"检查信息",url:"/aft/otherCheck/expireCheck_apply.jsp?checkId="+expireCheckId+"&proFlag="+proFlag}
				]
			},
	...
	//意见提交
	if("-1"!=proFlag){
		menudata[menudata.length]={id:"意见",text:"意见",url:"/com.bos.bps.service.workFlowAdvice.flow?bizId=<%=request.getParameter("expireCheckId") %>&processInstId=<%=request.getParameter("processInstId")%>&isSrc=2"};
	} 
	...
	function checkBeforeSub(){
		var json = {"expireCheckId":expireCheckId};
		msg = exeRule("RAFT_0001","1",json);
   	    if(null != msg && '' != msg){
	   		nui.alert(msg);
	   		return "1";
   	    }
