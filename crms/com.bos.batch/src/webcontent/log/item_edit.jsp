...
<div id="form1" style="width:94%;height:auto;overflow:hidden;text-align:left">
	<input type="hidden" name="tbBatchSpLog" class="nui-hidden" />
	<fieldset>
  	<legend>
   	 <span>批量日志信息</span>
    </legend>
...
function initForm() {
	var json=nui.encode({"tbBatchSpLog":
		{"logId":
		"<%=request.getParameter("logId") %>"}});
	$.ajax({
        url: "com.bos.batch.queryLog.getTbBatchSpLog.biz.ext",
        type: 'POST',
        data: json,
        cache: false,
...
function save() {
	...
	$.ajax({
        url: "com.bos.pub.crud.updateTbBatchSpLog.biz.ext",
        type: 'POST',
        data: json,
        cache: false,
        contentType:'text/json',
