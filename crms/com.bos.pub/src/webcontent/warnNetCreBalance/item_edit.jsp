...
<!-- 
  ...
  - Description:TB_SYS_WARN_NET_CRE_BALANCE, com.bos.dataset.sys.TbSysWarnNetCreBalance-->
<head>
<%@include file="/common/nui/common.jsp" %>
</head>
<body>
<div id="form1" style="width:100%;height:auto;overflow:hidden;">
	<input type="hidden" name="item" class="nui-hidden" /><input type="hidden" name="item._entity" value="com.bos.dataset.sys.TbSysWarnNetCreBalance" class="nui-hidden" />
	<div class="nui-dynpanel" columns="4">
		<label>授信余额预警阀值（元）：</label>
		<input name="item.balanceWarnLimit" required="false" class="nui-textbox nui-form-input" vtype="maxLength:26" />

		<label>机构编码：</label>
		<input name="item.orgId" required="false" class="nui-textbox nui-form-input" vtype="maxLength:32" />

		<label>关联关系网客户数量：</label>
		<input name="item.relNetCusCount" required="false" class="nui-textbox nui-form-input" vtype="maxLength:8" />
...
function initForm() {
	var json=nui.encode({"item":{"netRelCreditBalanceId":"<%=request.getParameter("itemId") %>",
		"_entity":"com.bos.dataset.sys.TbSysWarnNetCreBalance"}});
	$.ajax({
        url: "com.bos.pub.TbSysWarnNetCreBalance.getItem.biz.ext",
        type: 'POST',
        data: json,
        cache: false,
...
function save() {
	...
	$.ajax({
        url: "com.bos.pub.TbSysWarnNetCreBalance.saveItem.biz.ext",
        type: 'POST',
        data: json,
        cache: false,
        contentType:'text/json',
