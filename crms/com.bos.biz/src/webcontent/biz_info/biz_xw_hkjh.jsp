...
<center>
	<div id="form" style="width:99.4%;height:auto;overflow:hidden;">
		...
		<div class="nui-dynpanel" columns="4" id="show">
			...
			<label class="nui-form-label">贷款起期：</label>
			<input id="dkqq" class="nui-datepicker nui-form-input" name="dkqq" dateFormat="yyyy-MM-dd"/>
		...
		<div class="nui-toolbar" style="border-bottom:0;text-align: left;" id="hkjhdiv">
			...
			<a class="nui-button" iconCls="icon-add" onclick="remove('hkjh')">删除</a>
		</div>
...
<script type="text/javascript">
	...
	function initPage(){
		...
		$.ajax({
            url: "com.bos.bizInfo.bizInfo.initHkjhPage.biz.ext",
            type: 'POST',
            data: json,
            cache: false,
            contentType:'text/json',
	...
	function dataChange(a){
		...
		if(hkjh.BJ==null ||hkjh.BJ==''||hkjh.BJ<0){
			nui.alert("值不合法！");
			return;
		}
		if(hkjh.AMT==null ||hkjh.AMT==''||hkjh.AMT<0){
			nui.alert("值不合法！");
			return;
		}
		
		if(hkjh.END_DATE==null ||hkjh.END_DATE==''){
			nui.alert("值不合法！");
			return;
		}
		var o = form.getData();
        ...
        if (row) {
        	nui.confirm("确定删除吗？","确认",function(action){
            	...
            	if(row.UUID){
					...
					var jxzq = nui.get("jxzq").getValue();
					var dkqq = nui.get("dkqq").getValue();
            		var json = nui.encode({"delqc":row.QC,"bj":bj,"hkqc":hkqc,"yearrate":yearrate,"jxzq":jxzq,"dkqq":dkqq,"uuid":row.UUID,"amountDetailId":row.AMOUNT_DETAIL_ID});
	            	$.ajax({
    ...
    function firstRepayDayChange(){
		...
		var json = nui.encode({"amountDetailId":amountDetailId,"schkq":schkq,"bj":bj,"hkqc":hkqc,"yearrate":yearrate,"jxzq":jxzq,"dkqq":dkqq});
    	$.ajax({
            url: "com.bos.bizInfo.bizInfo.changeFirstRepayDate.biz.ext",
            type: 'POST',
            data: json,
            contentType:'text/json',
            cache: false,
