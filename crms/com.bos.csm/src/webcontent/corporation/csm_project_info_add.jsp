...
<body>
	...
	<%="\"" + request.getParameter("relType") + "\""%>
		;
		if (relType == "thirdCrd") {
			nui.get("project.projectType").setValue("04");
			nui.get("project.projectType").setRequired(true);
			nui.get("project.projectType").setEnabled(false);
		}
		if (partyId) {
			nui.get("project.partyId").setValue(partyId);
		}
		var data;
		...
		function save() {
			...
			$.ajax({
				url : "com.bos.csm.corporation.Project.insertProject.biz.ext",
				type : 'POST',
				data : json,
				cache : false,
		...
		function typeCase() {
			...
			if (liveDevArea && buildScale) {
				if (parseFloat(liveDevArea) >= parseFloat(buildScale)) {
		...
		function checkProjectNameUnique(e) {
			...
			if (e.isValid) {
				...
				var msg = exeRule("PUB_0019", "1", projson);
				if (null != msg && '' != msg) {
					e.errorText = msg;
					e.isValid = false;
				}
