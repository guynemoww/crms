...
<table style="width:100%;height:auto;table-layout:fixed;" class="nui-form-table">
						<tr>
							...
							<td>
<input class="nui-hidden nui-form-input" name="param"/>
<input class="nui-textbox nui-form-input" name="param.paramName" vtype="maxLength:50" required="true" enabled="false"/>
					        </td>
							<td class="nui-form-label">参数类型：</td>
							<td>
<input id="param.paramType" name="param.paramType" required="true"
	class="nui-dictcombobox nui-form-input" dictTypeId="model_param_type" enabled="false"/>
					        </td>
						</tr>
						<tr>
...
<input class="nui-textarea nui-form-input" name="param.paramExpr" required="true" style="width:500px;height:150px;"/>
    ...
    <script type="text/javascript">
	    ...
	    var view = "<%=request.getParameter("v") %>";
if (view == "1") {
...
function initForm() {
	var json=nui.encode({"param":{"paramId":"<%=request.getParameter("paramId") %>"}});
	$.ajax({
            url: "com.bos.pub.model.param.getParam.biz.ext",
            type: 'POST',
            data: json,
            cache: false,
