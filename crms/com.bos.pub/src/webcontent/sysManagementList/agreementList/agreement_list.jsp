...
<div id="form1" style="width:100%;height:auto;overflow:hidden;">
	...
	<div class="nui-dynpanel" columns="4">
		<label>授权机构：</label>
		<input name="tbSysAgreementList.grantOrgCode" class="nui-buttonEdit" vtype="maxLength:10" onbuttonclick="selectEmpOrg" />

		<label>证件编码：</label>
		<input id="orgn" name="tbSysAgreementList.orgCode"  class="nui-textbox nui-form-input" vtype="maxLength:10" />
...
<div id="grid1" class="nui-datagrid" style="width:100%;height:auto" 
	url="com.bos.pub.sysManagementList.getTbSysAgreementListList.biz.ext"
	dataField="tbSysAgreementLists"
    ...
    function add() {
        nui.open({
            url: "pub/sysManagementList/agreementList/agreement_add.jsp",
            title: "新增", 
            width: 800, 
        	height: 500,
        	allowResize:true,
    ...
    function edit(v) {
        ...
        if (row) {
            nui.open({
                url: "pub/sysManagementList/agreementList/agreement_edit.jsp?id="+row.id+"&view="+v,
                title: "编辑", 
                width: 800,
        		height: 500,
                allowResize:true,
    ...
    function remove() {
        ...
        if (row) {
        	nui.confirm("确定删除吗？","确认",function(action){
            	if(action!="ok") return;
            	var json=nui.encode({"tbSysAgreementList":{"id":
            		row.id,version:row.version}});
                $.ajax({
                     url: "com.bos.pub.sysManagementList.delTbSysAgreementList.biz.ext",
	                type: 'POST',
	                data: json,
	                cache: false,
