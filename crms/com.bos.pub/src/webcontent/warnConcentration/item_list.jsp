...
<div id="form1" style="width:100%;height:auto;overflow:hidden;">
	<input type="hidden" name="item._entity" value="com.bos.dataset.sys.TbSysWarnConcentration" class="nui-hidden" />
	<h3>集中度预警参数</h3>
...
<div id="grid1" class="nui-datagrid" style="width:100%;height:auto" 
	...
	<div property="columns">
		...
		<div field="newLoanConLimit" headerAlign="center" allowSort="true" >新增贷款集中度预警阀值</div>
		<div field="oldLoanConLimit" headerAlign="center" allowSort="true" >存量贷款集中度预警阀值</div>
    ...
    function add() {
        nui.open({
            url: "pub/warnConcentration/item_add.jsp",
            title: "新增", 
            width: 800, 
        	height: 500,
        	allowResize:true,
    ...
    function edit(v) {
        ...
        if (row) {
            nui.open({
                url: "pub/warnConcentration/item_edit.jsp?itemId="+row.concentrationId+"&view="+v,
                title: "编辑", 
                width: 800,
        		height: 500,
                allowResize:true,
    ...
    function remove() {
        ...
        if (row) {
        	nui.confirm("确定删除吗？","确认",function(action){
            	if(action!="ok") return;
            	var json=nui.encode({"item":{"concentrationId":
            		row.concentrationId,
					"_entity":"com.bos.dataset.sys.TbSysWarnConcentration"}});
                $.ajax({
                     url: "com.bos.pub.TbSysWarnConcentration.delItem.biz.ext",
	                type: 'POST',
	                data: json,
	                cache: false,
