...
<div id="form1" style="width:100%;height:auto;overflow:hidden;">
	<%--   --%><input type="hidden" name="tbSysBigCompanyList" class="nui-hidden" />
	<input type="hidden" name="tbSysBigCompanyList.id" class="nui-hidden" />
	<div class="nui-dynpanel" columns="4">
		<label>客户名称：</label>
		<input id="cus" name="tbSysBigCompanyList.custName" required="true" class="nui-buttonedit nui-form-input" onbuttonclick="onButtonEdit" vtype="maxLength:200" enabled="false"/>

		<label>授权机构：</label>
		<input name="tbSysBigCompanyList.grantOrgCode" required="true" class="nui-buttonEdit" vtype="maxLength:10" onbuttonclick="selectEmpOrg" dictTypeId="org"/>

		<label>证件编码：</label>
		<input id="orgn" name="tbSysBigCompanyList.orgCode" required="false" class="nui-textbox nui-form-input" vtype="maxLength:10" onvalidation="checkOrgnNum" enabled="false"/>
		<label>证件类型：</label>
...
function initForm() {
	var json=nui.encode({"tbSysBigCompanyList":
		{"id":
		"<%=request.getParameter("id") %>"}});
	$.ajax({
        url: "com.bos.pub.sysManagementList.getTbSysBigCompanyList.biz.ext",
        type: 'POST',
        data: json,
        cache: false,
        ...
        success: function (text) {
        	if(text.msg){
        		...
        		form.setData(text);
        		nui.get("cus").setText(text.tbSysBigCompanyList.custName);
        	}
        },
...
function save() {
	...
	$.ajax({
        url: "com.bos.pub.sysManagementList.updateTbSysBigCompanyList.biz.ext",
        type: 'POST',
        data: json,
        cache: false,
        contentType:'text/json',
