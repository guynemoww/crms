...
<div id="tabs1" class="nui-tabs" activeIndex="0" style="width:100%;height:110%;" onbeforeactivechanged="tabChange" >
	...
	<div class="nui-toolbar" style="border-bottom:0;width:99.5%;text-align: left;">
		<a class="nui-button" iconCls="icon-add" onclick="link()" id="link">新增</a>
		<a class="nui-button" iconCls="icon-save" onclick="update()" id="update">修改</a>
	...
	<script type="text/javascript">
		...
		function save() {
			...
			//20171026，陈研菲 要求增加，如果权证已出库，可以再次录入
			var jsonjson =  nui.encode({"suretyKeyId":suretyKeyId,"regNo":nui.get("tbGrtRegcardinfo.registerCertiNo").getValue()});
			$.ajax({
	        	url: "com.bos.grt.regmanage.collateralinandoutin.checkOutIfOk.biz.ext",
	        	type: 'POST',
	        	data: jsonjson,
	        	cache: false,
	        	async: false,
		...
		function addSuretyList() {
			var tabs = nui.get("tabs1");
			var tab = {title: "押品信息",name: "collateral",url: nui.context+"/grt/manage/collateral/collateral_list.jsp?"};
			tab = tabs.addTab(tab);            
		}
