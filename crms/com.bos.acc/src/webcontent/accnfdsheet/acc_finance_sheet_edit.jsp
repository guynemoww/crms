...
<body>
	<center>
		<div id="form1" style="width: 98%; height: auto; overflow: hidden; text-align: center; margin-top: 0px;">
			<div id="dw" style="height: auto; overflow: hidden; text-align: right;">
				<label>金额单位：元</label>
			</div>
			<div id="dwa" style="height: auto; overflow: hidden; text-align: left; color: red">
				<label id="xs" style="display: none;">百分数请换算成小数</label>
			</div>
			<input name="financeId" value="<%=request.getParameter("financeId")%>" class="nui-hidden" />
			<input name="financeStatementId" value="" class="nui-hidden" id="financeStatementId" />
			<div id="balanceDiv" style="width: 98%; height: auto; overflow: hidden; text-align: left; border: 1px solid black"></div>
			<div class="nui-toolbar" style="width: 98%; height: auto; overflow: hidden; text-align: right">
	...
	<script type="text/javascript">
		...
		function showBalanceSheet() {
			...
			//构造表单
			if (sheetCode == '02' || sheetCode == '03') {//利润、现金流量
			...
			for (var i = 0, len = logData.length; i < len; i++) {
				...
				if (log.itemType == '1') {
					...
					if (log.itemType == '3') {
						html += '<div style="width:100%;background-color: yellow;text-align:left;font-weight: bold">'
								+ "　　　　" + log.itemName;
						//		 html += '<label>'+log.itemName+'</label>';
						html += '</div>';
					} else {
					...
					if (sheetCode == '02' || sheetCode == '03') {//利润表、现金流量表--1202修改后其实是年报非年报顺序一样
						//本年累计值
						if (projectValue == 0) {
							html += '<input name="KM'
									+ log.itemCode
									+ '" class="nui-textbox nui-form-input" value="'
									+ projectValue
									+ '" vtype="maxLength:20" dataType="float" required="true" decimalPlaces="2"'
									+ viewFlag;
						} else if (projectValue == null || projectValue == '') {
							html += '<input name="KM'
									+ log.itemCode
									+ '" class="nui-textbox nui-form-input" vtype="maxLength:20" dataType="float" required="true" decimalPlaces="2"'
									+ viewFlag;
						} else {
							html += '<input  name="KM'
									+ log.itemCode
						...
						if (preFlag == '1') {//上期值取到且未保存过期初值
							...
							if (financeData['e' + log.itemCode] == 0) {
								html += '<input name="EM'
										...
										+ '" vtype="maxLength:20" dataType="float" required="true" decimalPlaces="2"'
										+ viewFlag;
							} else if (financeData['e' + log.itemCode] == null
									|| financeData['e' + log.itemCode] == '') {
								html += '<input name="EM'
										+ log.itemCode
										+ '" class="nui-textbox nui-form-input" vtype="maxLength:20" dataType="float" required="true" decimalPlaces="2"'
		...
		function save() {
			...
			//只有财报补充资料才校验必填
			if ('05' == sheetCode && form.isValid() == false) {
				alert("请将信息填写完整");
				return;
			}

			var json = nui.encode({
				financeStatementData : o
			});
			...
			if (msg != '' && msg != null) {
				msg = msg + '为空，';
				nui.confirm('有部分科目未填' + '您确定要保存数据吗？','确认',
					function(e) {
						if (e == 'ok') {
							$.ajax({
