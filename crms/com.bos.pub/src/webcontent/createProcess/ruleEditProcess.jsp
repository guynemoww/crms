...
<div id="form1" style="width:100%;height:auto;overflow:hidden;">
	...
	<div class="nui-dynpanel" columns="4">
		...
		<label>规则结果类型：</label>
		<input name="item.resulttype" required="false" class="nui-dictcombobox nui-form-input" dictTypeId="pub_grant_param" />

		<label>引用规则引用标识：</label>
		<input name="item.rrind" required="false" class="nui-text nui-form-input" vtype="maxLength:200" />

		<label>规则状态：</label>
		<input name="item.rstatus" required="false" class="nui-text nui-form-input" dictTypeId="put_grant_pack_status" />
...
<div class="nui-toolbar" style="border-bottom:5;text-align:center;" >
	<a class="nui-button" iconCls="icon-save" onclick="abortProcess()">撤销流程</a>
</div>
...
function initForm() {
	var json=nui.encode({"item":{"rid":"<%=request.getParameter("bizId") %>"}});
	$.ajax({
...
function editRule() {
	...
	var json=nui.encode({'processInstId':'<%=request.getParameter("processInstId") %>','processDefName':'<%=request.getParameter("processDefName") %>','activityDefId':'<%=request.getParameter("activityDefId") %>','templateVersion':'<%=request.getParameter("templateVersion") %>'});
		$.ajax({
            url: "com.bos.pub.decision.abortProcess.biz.ext",
            type: 'POST',
            data: json,
            cache: false,
            contentType:'text/json',
            success: function (text) {
            	if(text.msg){
            		nui.alert(text.msg);
            		   ...
            		   nui.alert("撤销成功!点击确定返回待办工作列表","提示",function(action){
