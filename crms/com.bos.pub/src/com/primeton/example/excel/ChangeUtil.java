...
public class ChangeUtil {
	...
	public static java.sql.Date changeToDBDate(String _dateStr)
			throws IllegalArgumentException {
		return changeForDBDate(changeToDate(_dateStr, DATE_FORMAT_YMD_HMS));
	}
	...
	public static String getBigNum(long num){
		String bigNum = "";
		String[] bigNums = new String[]{"零", "壹", "贰", "叁", "肆", "伍", "陆", "柒", "捌", "玖"};//大写数字
		//单位
		String[] bigUnits = new String[]{"", "拾", "佰", "仟", "万", "拾", "佰", "仟", "亿", "拾", "佰", "仟", "兆" ,"拾", "佰", "仟"}; 
		
		//获取数字的最高位
		int len = (num+"").toCharArray().length;
		//添加是否添加"零"到大写数字中
		boolean flag = false;//数字中的"0"是否读取的标记
		int i = 0;//记录读到哪一位
		while(true){
			if(i==len){
				break;
			...
			//每次取最低位
			int yushu = (int)(num%10);
			if(yushu!=0){//当前读取位的数字不为“0”时要带单位
				int unit = i/4;//要取节点单位的位置（万，亿，兆）
				if(i%4!=0&&!flag&&!bigNum.contains(bigUnits[unit*4])){//当前位不为"0"并且不是单位节点位,在节点单位内（4个一组）比当前位低的都为"0"时加上节点单位
					bigNum = bigUnits[unit*4]+bigNum;
				}
				//将当前位数字转化为中文并加上单位
				bigNum=(bigNums[yushu]+bigUnits[i])+bigNum;
				flag = true;//比当前位高位的"0"是否读取的标记
			}else{//当前位数字为"0"
				if(i%4!=0&&flag){
					bigNum = bigNums[yushu]+bigNum;
				}
				flag = false;//转换一次"0"
			}
			i++;
