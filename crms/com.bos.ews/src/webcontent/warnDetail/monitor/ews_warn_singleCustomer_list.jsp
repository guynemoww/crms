...
<div class="nui-toolbar" style="margin-top:40px;border-bottom:0;text-align:right;">
	    ...
	    <a class="nui-button"  iconCls="icon-add" onclick="viewInfo()">新增信号</a>
</div>
...
function addCsm(){
  git.mask();
  var url=nui.context+"/ews/warnDetail/monitor/ews_new_singleCsm_list.jsp";
  git.go(url);              
}       
...
function viewInfo(){
   ...
   return;
     ...
     var url= nui.context+ "/ews/warnDetail/warnInfo/ews_warnInfo_monitor_add.jsp?corpid="+row.partyId+"&type=01&partyName="+row.partyName+"&partyNum="+row.partyNum;
     git.go(url);        
