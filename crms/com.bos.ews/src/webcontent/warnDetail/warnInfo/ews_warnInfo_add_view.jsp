...
<!-- <div style="margin-top:30px;font-weight: bolder;">预警信号类别</div> -->
<fieldset>
  	<legend>
    	<span>预警信号类别</span>
    </legend>
...
<fieldset>
  	<legend>
    	<span>已加入流程中的待审核预警信号列表(含自动预警信号)</span>
    </legend>
<div id="grid" class="nui-datagrid" style="width:100%;height:auto;margin-top: 7px;" url="com.bos.ews.warnInfo.queryEwsInfo.biz.ext" allowCellSelect="true" allowCellEdit="true"  dataField="csmWarnInfo"
	 sizeList="[10,20,30,40]"  pageSize="10" multiSelect="true" allowResize="true" showReloadButton="false" showPageSize="true" multiSelect="false" sortMode="client" onload="onload">
     <div property="columns">
...
<div class="nui-toolbar" style="border-bottom:0;text-align:center;margin-top: 7px;">
     ...
     <div id="saveData" class="nui-button" iconCls="icon-remove" onclick="del()" style="float: right;">删除信号</div>
</div>
<fieldset>
  	<legend>
    	<span>是否认定级别</span>
    </legend>
<div id="level" class="nui-dynpanel" columns="3" style="margin-top:20px;text-align:left;border: 0px;">
	 ...
	 <div id="saveLevelEdit" class="nui-button" iconCls="icon-save" onclick="updateEdit()" style="float: right;">保存修改</div>
</div>
</fieldset>
...
<div id="toolbar" class="nui-toolbar" style="margin-top:20px;border-bottom:0;text-align:center;">
     <a class="nui-button" iconCls="icon-save" id="btnSave" style="float: right;">保存修改</a>
</div>
 -->
<script type="text/javascript">
	    ...
	    //var form3 = new nui.Form("#form3");                                   //预警信号建议表单
		...
		var isLevelEdit = "<%=request.getParameter("isLevelEdit") %>";        //是否发起级别认定
	    var adjust ;                                                          //预警变更实例
		...
		if(isLevelEdit=="0"||isLevelEdit=="1"){
		  nui.get("startHold").setValue(isLevelEdit);
		}
		var callType = "";                                                    
		git.mask();
                   // getAdjust();
...
function updateEdit(){
         git.mask();
         var rslt = nui.get("startHold").value;
...
function formSetData(){
         var data = nui.encode({bizId:bizId});
                    $.ajax({                                                         
                           url: "com.bos.ews.util.getAdjust.biz.ext",
                           type: 'POST',
                           data: data,
                           cache: false, 
                           contentType:'text/json',
...
function save(){
    ...
    var json=nui.encode({"corpid":"<%=request.getParameter("corpid") %>","ob":productArray,"status":4,bizId:bizId,inflow:"1"});
    $.ajax({                                                         
...
//刷新頁面
function reload(){
var url=nui.context+"/ews/warnDetail/warnInfo/ews_warnInfo_add_view.jsp?corpid="+corpid+"&type="+type+"&flowType=1&bizId="+bizId+"&processInstId="+processInstId;
git.go(url);
}
...
修改预警变更信息
function saveUpdate(){
     var suggestState = nui.get("suggestState").getValue();
     //var matter = nui.get("matter").getValue();
     var adjust ={"levelAdjustId":bizId,"matterState":matter,"suggestState":suggestState};
     var json={adjust:adjust};
    nui.ajax({
                url: "com.bos.ews.warnInfo.updateAdjust.biz.ext",
                data:json,
                type:"POST",
                contentType:'text/json',
                success: function (text) {
