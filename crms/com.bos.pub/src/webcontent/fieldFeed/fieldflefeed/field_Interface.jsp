...
function initParams() {
	...
	// data.item.pid=pid;
		$.ajax({
	       ...
	       type: 'POST',
	       data: nui.encode({"field":{"recordeType":"<%=request.getParameter("recordeType") %>","isShow":"1"}}),
	       cache: false,
...
function initParamDom(item) {
	...
	form.setData(item);
	if ("<%=request.getParameter("proFlag") %>"=="0") {
	...
	$.ajax({
            url: "com.bos.pub.fieldflefeed.getFieldValueList.biz.ext",
            type: 'POST',
            data:  nui.encode({"field":{"recordeType":"<%=request.getParameter("recordeType") %>","partyId":"<%=request.getParameter("ids") %>"}}),
            cache: false,
            contentType:'text/json',
            success: function (text) {
            		if(text.fieldRecords.length<0){
            		}else{
            		  for(var i=0;i<text.fieldRecords.length;i++){
            		     nui.get(text.fieldRecords[i].recordeField).setValue(text.fieldRecords[i].recordeValue);
...
function save() {
	...
	for (p in dat) {
		...
		if (!type)
			continue;
		arr[arr.length]={name: p.split(":")[0],chName: p.split(":")[1],val: dat[p], type: type, partyId: "<%=request.getParameter("ids") %>" ,recordeType:"<%=request.getParameter("recordeType") %>"};
	}
	var json = nui.encode({items:arr,recordeType:"<%=request.getParameter("recordeType") %>",partyId:"<%=request.getParameter("ids") %>"});
	$.ajax({
	    url: "com.bos.pub.fieldflefeed.addFieldValues.biz.ext",
	    type: 'POST',
	    data: json,
	    cache: false,
	    contentType:'text/json',
