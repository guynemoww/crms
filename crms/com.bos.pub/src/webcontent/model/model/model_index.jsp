...
<body>
		...
		<div id="grid1" class="nui-datagrid" style="width:100%;height:auto" 
		    url="com.bos.pub.model.model.getModelIndexList.biz.ext" dataField="items"
...
function initForm() {
	nui.get("grid1").load({"itemId":"<%=request.getParameter("itemId") %>"});
}
initForm();

function addItem() {
	var g=nui.get("grid1");
    var newRow = { name: "New Row","tbPubModel":{"modelId":"<%=request.getParameter("itemId") %>"} };
	g.addRow(newRow, 0);
}
function delItem() {
	var g=nui.get("grid1");
            ...
            ondestroy: function (action) {            
                if (action == "ok") {
                    ...
                    if (data) {
                        ...
                        //alert(nui.encode(g.getSelected()));
                        //g.getSelected().miName=data.indexInd;
...
function modelIndex(v) {
            ...
            if (row) {
                ...
                nui.open({
url: "<%=request.getContextPath() %>/pub/model/model/model_index_sub.jsp?modelId=<%=request.getParameter("itemId") %>&pMiId="
                    	+row.miId+"&view="+v+"&miName="+encodeURI(row.miName),
                    title: "模型指标", 
                    width: 800,
            		height: 500,
                    allowResize:true,
