...
<form id="uploadFileForm" action="com.bos.pub.document.updown.upload.biz.ext2" enctype="multipart/form-data" method="post">
    ...
    <input type="hidden" name="bizPhase" value="irm" /> 
	<input id="docType" class="nui-hidden nui-form-input" name="docType" />
<%--<input type="hidden" name="docName" value="业务申请文件" />	--%>
	<input type="hidden" name="docNote" value="备注或描述信息" />
	<input type="hidden" name="redirectPage" value="/irm/singleCustom/creditRate/relevantFile.jsp?applyId=<%=request.getParameter("applyId")%>+&docType=<%=request.getParameter("docType")%>" />
	    ...
	    //$.ajax({
	     ...
	     //   type: 'POST',
	      ...
	      //  success: function (mydata) {
	         ...
	         //   form.setData(o);
	            nui.get("bizNum").setValue(applyId);
	            nui.get("docType").setValue(docType);
	            search();
	        //}
	 //   });
	    function search() {
		     ...
		     switch (corpType){ 
                              case 1:   
                                        ...
                                        bizUrl ="com.bos.aft.aft_small_inspect.downloadSmallBusiReport.biz.ext2"; 
                                         break; 
                              case 2:
                                        nui.encode({irId:"<%=request.getParameter("applyId")%>","pageName":"inspectReportPage"}) ;
