...
<div id="layout" class="nui-layout" style="width:100%;height:100%;">
	<div title="业务申请3" region="west"  width="240" class="sub-sidebar" allowResize="false">
    ...
    $.ajax({
        ...
        success: function (mydata) {
 				...
 				menudata = [
					{id:"贷款结构", text:"贷款结构", //expanded:true,  
						children:[
							...
							{id:"额度明细",text:"额度明细",url:"/biz/biz_info/biz_gs_detail_zh.jsp?applyId="+applyId+"&proFlag="+proFlag+"&bizType="+o.tbBizApply.bizType},
							{id:"监管报送信息",text:"监管报送信息",url:"/biz/biz_info/biz_gs_jgbs.jsp?applyId="+applyId+"&proFlag="+proFlag}
					]}
				];
			}
			...
			if("03"!=o.tbBizApply.bizType){
				...
				menudata[menudata.length]={id:"岗位责任",text:"岗位责任",url:"/biz/biz_info/biz_advice.jsp?applyId="+applyId+"&proFlag="+proFlag}
			...
			//意见提交
			if("-1"!=proFlag){
				menudata[menudata.length]={id:"意见",text:"意见",url:"/com.bos.bps.service.workFlowAdvice.flow?bizId="+applyId+"&processInstId=<%=request.getParameter("processInstId")%>&isSrc=2"};
			}
			git.unmask();
		            	...
		            	if('0' == ifFlag){
					    	...
					    	$.ajax({
							        ...
							        success: function (data) {
							        	...
							        	var url = "http://"+"<%=ipStr%>"+":"+"<%=portStr%>"+"/default/com.bob.bcms.collateralmgr.CollMgr.flow?creditFlag=1&orgId=<%=((UserObject)session.getAttribute("userObject")).getUserOrgId()%>&userId=<%=((UserObject)session.getAttribute("userObject")).getUserId()%>&applyId="+applyId+"&collType=01&oldApplyId="+oldApplyId+"&proFlag="+proFlag+"&cltFlag=01&ifApp=1&senceCode=1";
							       		window.open(url);
										return;
							        },
							        error: function (jqXHR, textStatus, errorThrown) {
	            	...
	            	if('1' == ifFlag){
            			$.ajax({
						        ...
						        success: function (data) {
						        	...
						        	var url = "http://"+"<%=ipStr%>"+":"+"<%=portStr%>"+"/default/com.bob.bcms.collateralmgr.CollMgr.flow?creditFlag=1&orgId=<%=((UserObject)session.getAttribute("userObject")).getUserOrgId()%>&userId=<%=((UserObject)session.getAttribute("userObject")).getUserId()%>&applyId="+applyId+"&collType=02&oldApplyId="+oldApplyId+"&proFlag="+proFlag+"&cltFlag=02&ifApp=1&senceCode=1";
						       		window.open(url);
									return;
						        },
						        error: function (jqXHR, textStatus, errorThrown) {
