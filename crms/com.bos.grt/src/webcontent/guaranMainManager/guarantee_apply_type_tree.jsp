...
<script type="text/javascript">
	...
	function save(){
		...
		if("020406" == currentNode.dictID){
			nui.open({
				url: nui.context+"/grt/guaranMainManager/guarantee_apply_list_guaranteer_add.jsp?sortType="+currentNode.dictID+
				"&applyId=<%=request.getParameter("applyId") %>&bizCustType=<%=request.getParameter("bizCustType") %>",
				title: "新增"+contitle, 
				width: 800, 
				height: 500,
				allowResize:false,
		...
		}else if("020405" == currentNode.dictID){//如果是履约责任保证保险（020407）需要录入详细履约责任保证保险信息
			nui.open({
				url: nui.context+"/grt/guaranMainManager/guarantee_apply_list_creditsafe_add.jsp?sortType="+currentNode.dictID+
				"&applyId=<%=request.getParameter("applyId") %>&bizCustType=<%=request.getParameter("bizCustType") %>",
				title: "新增"+contitle,
				width: 800, 
				height: 500,
			...
			nui.open({
				url: nui.context+"/grt/guaranMainManager/guarantee_apply_list_basic_add.jsp?sortType="+currentNode.dictID+
				"&applyId=<%=request.getParameter("applyId") %>&bizCustType=<%=request.getParameter("bizCustType") %>",
				title: "新增"+contitle, 
				width: 800, 
				height: 500,
