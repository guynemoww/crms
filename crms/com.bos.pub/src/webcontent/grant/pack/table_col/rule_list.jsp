...
<div id="form1" style="width:100%;height:auto;overflow:hidden;">
	<input type="hidden" name="item._entity" value="com.bos.pub.decision.SelectRuleListByPid" class="nui-hidden" />
	<input type="hidden" id="item.pid" name="item.pid" value="<%=request.getParameter("pid") %>" class="nui-hidden" />
	<div class="nui-dynpanel" columns="6">
		...
		<label>规则分类：</label>
		<input id="item.ruletype" name="item.ruletype" required="true" 
...
<div id="grid1" class="nui-datagrid" style="width:100%;height:auto" 
	url="com.bos.pub.decision.getRuleListByPid.biz.ext" dataField="items"
    ...
    <script type="text/javascript">
		...
		$.ajax({
		    ...
		    success: function (text) {
		    	...
		    	nui.get("item.ruletype").setData(grant_packages);
		    	if (grant_packages.length>0) {
		    		nui.get("item.ruletype").setValue(grant_packages[0].pid);
		    	}
		    	search();
		    },
		    error: function () {
	...
	initPackages();
      ...
      //  search();
        ...
        function reset(){
			form.reset();
			nui.get("pid").setValue("<%=request.getParameter("pid") %>");
			search();
		}
