...
<body>
  	...
  	<form id="importForm" action="com.bos.grt.grtImportExcel.ImportTbBizPjxxApply.flow" method="post" enctype="multipart/form-data" >
  	<input name="amountDetailId1" id="amountDetailId1" class="nui-hidden" />
  	<input name="contractNum" id="contractNum" class="nui-hidden" />
  	<input name="contractId" id="contractId" class="nui-hidden" />
...
<script type="text/javascript">
	nui.parse();
 	...
 	//下载模板
 	function downExcel(){
 		f.action = "com.bos.grt.grtImportExcel.ImportTbBizPjxxApply.flow?_eosFlowAction=downExcel";
	 	f.submit();
 	}
  /*
  * 功能：导入表内
 ...
 function empImport(){
	if (amountDetailId) {
		nui.get("amountDetailId1").setValue(amountDetailId);
	}
	if (contractNum) {
		nui.get("contractNum").setValue(contractNum);
	}
	if (contractId) {
		nui.get("contractId").setValue(contractId);
	}
  	...
  	f.action = "com.bos.grt.grtImportExcel.ImportTbBizPjxxApply.flow?_eosFlowAction=importFile";
  	f.submit();
 }
