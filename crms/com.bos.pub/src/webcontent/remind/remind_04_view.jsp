...
<script type="text/javascript">
       		...
       		for (var i=0; i<e.data.length; i++){
	   			...
	   			if('02'==status){
					...
					if(e.data[i]['TEAM_USER_NUM'] == userId){
						//如果管户经理是当前登录人 则可以操作
						e.data[i]['op']='<a href="#" onclick="toGo(\''+e.data[i].RECEIPT_NUM+'\');">催收登记</a> ';
					}   			
	   			}
	   			var res = getReceiptInfo(e.data[i]['RECEIPT_NUM']);
	   			if(!res||!res.RECEIPT_NUM){
	   				continue;
	   			}
	...
	function toGo(receiptNum){
		...
		nui.open({
            url: nui.context+"/clt/register/clt_loan_summary_list.jsp?receiptNum="+receiptNum,
            title: "催收登记", 
            width: 1024,
    		height: 768,
    		state:"max",
