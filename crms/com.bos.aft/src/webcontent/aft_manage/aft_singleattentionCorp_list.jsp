...
<body>
	...
	<div id="topTitle" title="移入贷后重点客户" >
	<center>
	<div id="queryForm" class="nui-form" style="width:99.5%;height:auto;overflow:hidden;">
		<div class="nui-dynpanel" columns="6" id="queryCsm">
			...
			<label>客户类型：</label>
			<input name="partyTypeCd"  class="nui-dictcombobox nui-form-input" dictTypeId="XD_KHCD0219" value=""/>
	...
	<div id="datagrid" class="nui-datagrid" style="width:99.5%;height:auto;" 
		    ...
		    sizeList="[10,20,50,100]" url="com.bos.aft.aft_manage.queryMonitorCsmCorp.biz.ext" dataField="csmCorps">
	...
	<script type="text/javascript">
		...
		if(type=="monitoring"){
			 //alert();
			document.title="移入贷后重点监控客户";
			$("#topTitle").text("移入贷后重点监控客户");
		}
		...
		function saveCsm(){
			...
			var json=nui.encode({"partyId":rows.partyId});
			var url="";
           	if(type=="attention"){
           		url=nui.context + "/aft/aft_manage/aft_addattentionCorp_list.jsp?rows="+nui.encode(rows);
           		$.ajax({
					url: "com.bos.aft.aft_manage.repeatAttentionCorp.biz.ext",
            		type: 'POST',
            		data: json,
            		cache: false,
            		contentType:'text/json',
           			success: function (data) {
           				if(data.reapyCount==0){
				...
				url=nui.context + "/aft/aft_manage/aft_addmonitoringCorp_list.jsp?rows="+nui.encode(rows);
				$.ajax({
					url: "com.bos.aft.aft_manage.repeatMonitoringCorp.biz.ext",
            		type: 'POST',
            		data: json,
            		cache: false,
            		contentType:'text/json',
