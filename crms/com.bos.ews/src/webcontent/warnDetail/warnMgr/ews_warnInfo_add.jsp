...
<fieldset>
  	<legend>
    	<span>11待提交流程审核的预警信号列表(含自动预警信号)</span>
    </legend>
<div id="grid" class="nui-datagrid" style="width:100%;height:auto;margin-top: 7px;" url="com.bos.ews.warnMgr.queryWarnInfoInFlow.biz.ext" allowCellSelect="true" allowCellEdit="true"  dataField="csmWarnInfo"
     ...
     <div property="columns">
         ...
         <div field="signalState" headerAlign="center" allowSort="true">
             <input property="editor" class="nui-textarea nui-form-input" required ="true" />备注
         </div>
     </div>
</div>
</fieldset>
...
<div id="toolbar" class="nui-toolbar" style="margin-top:20px;border-bottom:0;text-align:center;">
     <a class="nui-button" iconCls="icon-save" onclick="startFlow()" id="btnSave" style="float: right;">提交流程审核</a>
</div>
...
<script type="text/javascript">
		...
		var callType = "";
		var bizId= "<%=request.getParameter("bizId")%>";                        //业务ID
/*修改单元格*/	    
...
function getAllWarnInfo(e,f){
          ...
          var json=nui.encode({dictid:dictid,dictid2:dictid2,dicttypeid:"XD_YJCD0003",partyId:corpid});
                    $.ajax({                                                         
...
/*初始化方法*/
 function initForm() {
                    ...
                    git.mask();
                   // getAdjust();
                   if(bizId==null||typeof(bizId)=="undefined"){
                      grid.load({partyId:corpid,status:"6"});
                   }else{
                      grid.load({partyId:corpid,bizId:bizId,status:"7"});
                   }
                    
            	    if(type == "06"){
            	         //alert("担保类信号："+type);
            	         grid1.load({dicttypeid:"XD_YJCD0003",dictid:"23____",partyId:corpid,oneQuery:1});//加载担保类型预警信号
            	         callType = "1";//getAllWarnInfo("23%");                                          //加载当前相关的所有信号
            	    }else if(type == "05"){
            	         //alert("集团类信号："+type);
...
function save(){
    ...
    if(productArray.length==0){
     ...
     var url;
     if(bizId==null||typeof(bizId)=="undefined"){
                       json=nui.encode({"corpid":"<%=request.getParameter("corpid") %>","ob":productArray,"status":6});   //6表示总行认定岗新增信号状态
                       url="com.bos.ews.warnMgr.addWarnInfo.biz.ext";
                   }else{
                       json=nui.encode({"corpid":"<%=request.getParameter("corpid") %>","ob":productArray,"status":7,bizId:bizId});   //6表示总行认定岗新增信号状态
...
//刷新頁面
function reload(){
var url=nui.context+"/ews/warnDetail/warnMgr/ews_warnInfo_add.jsp?corpid=<%=request.getParameter("corpid") %>&type="+type;
git.go(url);
}
...
function startFlow(){
            ...
            if(rows.length==0){                                       //没有选中项，表单不提交。
			   ...
			   return;
                ...
                var json=nui.encode({"corpid":"<%=request.getParameter("corpid") %>","ob":rows});
                $.ajax({                                                         
                      url: "com.bos.ews.warnMgr.addWarningInfo.biz.ext",
                      type: 'POST',
                      data: json,
                      cache: false,
                      contentType:'text/json',
