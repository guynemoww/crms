...
<div id="panel1" class="nui-panel" title="抵押"
	...
	<div id="datagrid1" class="nui-datagrid" style="width:100%;height:auto;"  sortMode="client"
		...
		<div property="columns">
			...
			<div field="partyName" allowSort="true"  width="" headerAlign="center">抵押人名称</div>
	        <div field="sortType" allowSort="true" width="" dictTypeId="XD_DBCD4002" headerAlign="center">抵押物类型</div>
	        ...
	        <div field="mortgageRate" allowSort="true" width="" headerAlign="center">抵押率(%)</div>
	     </div>
	</div>

	<div class="nui-toolbar" style="text-align:right;padding-top:15px;padding-right:25px;"borderStyle="border:0;">
    	<a class="nui-button" iconCls="icon-add" id="biz_single_mortgage_add" onclick="addAppGerenal">增加</a>
    	<a class="nui-button" iconCls="icon-add" id="biz_single_mortgage_insert" onclick="insert">引入</a>
    	<a class="nui-button" iconCls="icon-edit" id="biz_single_mortgage_edit" onclick="edit(0)">编辑</a>
		<a class="nui-button" iconCls="icon-edit" onclick="edit(1)">查看</a>
		<a class="nui-button" iconCls="icon-remove" id="biz_single_mortgage_remove" onclick="remove()">删除</a>
	</div>
</div>
<div id="panel1" class="nui-panel" title="最高额担保合同关联"
	...
	showCollapseButton="true" showFooter="false" allowResize="false">
		...
		<div class="nui-toolbar" style="text-align:right;padding-top:15px;padding-right:25px;"borderStyle="border:0;">
	    	<a class="nui-button" iconCls="icon-add" id="biz_single_highMor_insert" onclick="insert2">引入</a>
	    	<a class="nui-button" iconCls="icon-edit"  onclick="edit2(1)">查看</a>
			<a class="nui-button" iconCls="icon-remove" id="biz_single_highMor_remove" onclick="remove2()">删除</a>
		</div>
</div>
...
<script type="text/javascript">
    ...
    function search() {
        ...
        grid.load({"item":{"applyId":"<%=request.getParameter("bizId") %>","_entity":"com.bos.dataset.biz.TbBizGrtRelation","reType":"01"}});
    }
    search();
	
	<%-- nui.get("biz_single_mortgage_add").hide();
  	  nui.get("biz_single_mortgage_edit").hide();
  	  nui.get("biz_single_mortgage_remove").hide();
  	  //nui.get("biz_single_mortgage_insert").hide();
    if("<%=request.getParameter("type") %>"=="04"){
    	nui.get("biz_single_mortgage_add").hide()
    	nui.get("biz_single_mortgage_edit").show();
  	  	nui.get("biz_single_mortgage_remove").show();
  	  	check("0201");
    }
    	...
    	nui.get("biz_single_mortgage_add").show();
  	 	nui.get("biz_single_mortgage_edit").show();
  	  	nui.get("biz_single_mortgage_remove").show();
  	  	check("0201");
    }--%>
	            	...
	            	if(text.msg){
	            		...
	            		if(proFlag=="0"){
	            			nui.get("biz_single_mortgage_add").hide();
  	 						...
  	 						nui.get("biz_single_mortgage_remove").hide();
  	 						nui.get("biz_single_mortgage_insert").hide();
  	  						nui.get("biz_single_highMor_remove").hide();
  	   						nui.get("biz_single_highMor_insert").hide();
	            		}else{
		            		if(text.col.indexOf(colType)>=0){
		            			nui.get("biz_single_mortgage_add").show();
	  	  						nui.get("biz_single_mortgage_edit").show();
	  	 						nui.get("biz_single_mortgage_remove").show();
	  	 						nui.get("biz_single_mortgage_insert").show();
	  	  						nui.get("biz_single_highMor_remove").show();
	  	   						nui.get("biz_single_highMor_insert").show();
		            			return true;
		            		}else{
		            			nui.get("biz_single_mortgage_add").hide();
	  	  						nui.get("biz_single_mortgage_edit").hide();
    ...
    function addAppGerenal(){
    	 nui.open({
		   url:nui.context+"/grt/collateralparameter/colllsortparameter/selectCollateralSortInfoTree.jsp?collType=020101",
		   title:"请选择押品分类",
		   width:320,
		   height:360,
		   allowResize: false,
    ...
    function localGeneral(sortType,parentSortType){
    		nui.open({
				url: git.toUrlParam(nui.context+"/grt/grtMainManage/grt_apply_colmanage_add.jsp?sortType="+sortType+"&parentSortType="+parentSortType+"&applyId=<%=request.getParameter("bizId") %>&type=<%=request.getParameter("type") %>&collType=020101&reType=01&bizCustType=01&bizCustType=01"),
				title:"押品信息",
				width:800,
				height:600,
				allowResize: false,
	...
	/*function add(){
		nui.open({
            url: nui.context + '/grt/manage/TbGrtGeneralAppinfo/TbGrtGeneralAppinfo_add.jsp?bizId=<%=request.getParameter("bizId")%>&collType=020101&reType=01&bizCustType=01',
            showMaxButton: true,
            title: "添加押品信息",
            width: 950,
            height: 550,
	...
	function insert(v) {
            nui.open({
                url: nui.context + "/grt/grtMainManage/grt_apply_colmanage_import.jsp?applyId=<%=request.getParameter("bizId") %>&type=<%=request.getParameter("type") %>&collType=020101&reType=01&bizCustType=01",
                title: "引入押品", 
                width: 800,
        		height: 500,
                allowResize: false,
    ...
    function search2() {
        ...
        grid2.load({"item":{"applyId":"<%=request.getParameter("bizId") %>","bizCustType":"01","reType":"01"}});
    }
    search2();
    
    function insert2() {
            nui.open({
                url: nui.context + "/biz/biz_grt/selectMaxLoancon.jsp?applyId=<%=request.getParameter("bizId") %>&subcontractTypeCd=01&bizCustType=01&reType=01&partyId=<%=request.getParameter("partyId") %>",
                title: "引入最高额担保合同", 
                width: 800,
        		height: 500,
                allowResize:true,
