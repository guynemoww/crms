...
<body>
	<div id="form" style="width:98%;height:auto;overflow:hidden; text-align:center;margin: 10px;"  >
	...
	<div  class="nui-dynpanel" columns="4">
		...
		<label>保证金比例（%）：</label>
		<input id="subContract.bzjbl"name="subContract.bzjbl" required="false" enabled="false"class="nui-textbox nui-form-input"   />
		
		<label>保证金金额：</label>
		<input id="subContract.bzjje" required="true" name="subContract.bzjje"  class="nui-textbox nui-form-input" onblur="countBzjbl"vtype="maxLength:20;range:0,100000000000" dataType="currency"/>
...
<script type="text/javascript">
	...
	function initForm(){
		...
		var	json=nui.encode({"subContract":{"subcontractId":"<%=request.getParameter("subcontractId") %>"},"contractId":contractId});
		$.ajax({
	        ...
	        success: function (text) {
	        		...
	        		//国结的产品 放开对币种的控制
	        		if(text.contractInfo.productType.substring(0,5).indexOf("01007")!=-1){
...
// 			$("#endDateLab").show();
	...
	function countBzjbl(){
		...
		var bzjje=nui.get("subContract.bzjje").getValue();
		var bzjbl;
		if(contractAmt!=""&&bzjje!=""&&contractAmt!=0){
			bzjbl=parseFloat(bzjje)/parseFloat(contractAmt) * 100;
			nui.get("subContract.bzjbl").setValue(bzjbl.toFixed(2))
		}
	}
	</script>
</body>
