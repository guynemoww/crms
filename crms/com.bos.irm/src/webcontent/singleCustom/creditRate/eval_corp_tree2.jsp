...
<script type="text/javascript">
	...
	var flowType = "<%=request.getParameter("flowType") %>";//评级类型 01:新建，02：更新，03：再审核。
	var applyId;
	var reAud = "0";
	...
	init();	
	function init(){
		var temp = "<%=request.getParameter("flowType") %>";
		if (temp == "03"){
			reAud = "1";
		}
	...
	var url = "/bps/mywork/work_flow_advice.jsp?processInstId="+node.processInstId+"&processDefName="+node.processDefName;
		...
		url+="&workitemMappingId="+node.workitemMappingId+"&templateVersion="+node.templateVersion +"&doUrl="+node.doUrl+"&startTime="+node.startTime + "&isSrc=" +isSrc;	
	if(flg == 1){		
	var menudata = [
		{id:"内部评级", text:"内部评级", //expanded:true, 
			...
			{id:"评级结论",text:"评级结论",url:"/irm/singleCustom/creditRate/eval_corp_rateConclusion.jsp?partyId="+partyId+"&applyId="+applyId+"&reAud="+reAud+"&processInstId="+node.processInstId+"&oldApplyId="+oldApplyId+"&posicode="+posicode+"&flowType="+flowType},
