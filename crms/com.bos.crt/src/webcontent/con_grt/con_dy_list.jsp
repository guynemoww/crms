...
<div id="form" style="width:99.5%;height:auto;overflow:hidden; text-align:center;"  class="nui-form">
	...
	<div id="grid" class="nui-datagrid" style="width:100%;height:auto" 
		...
		<div property="columns">
			...
			<div field="SUBCONTRACT_NUM" headerAlign="center" allowSort="true" >抵押合同编号</div>
			<div field="PARTY_NAME" headerAlign="center" allowSort="true" >抵押人名称</div>
...
<script type="text/javascript">
	...
	//初始化查询
	function search(){
	    grid.load({"contractId":contractId,"subcontractType":subcontractType,"xgbz":xgbz});
	}
	//添加担保合同
	function add(){
		//添加担保合同信息
				            ...
				            success: function (mydata) {
								...
								$.ajax({
						            ...
						            success: function (mydata) {
					            		...
					            		nui.open({
								            url: nui.context + "/crt/con_grt/con_dy_tab.jsp?subcontractId="+o.conSubcontractRel.subcontractId+"&applyId="+applyId+"&contractType="+contractType+"&conSubconId="+o.conSubcontractRel.conSubconId,
								            showMaxButton: true,
								            title: "抵押合同明细信息",
								            width: 800,
								            height: 500,
    ...
    function edit(v){
        ...
        if( row.IF_TOP_SUBCON!="1"){
	        ...
	        nui.open({
	            url: nui.context + "/crt/con_grt/con_dy_tab.jsp?subcontractId="+row.SUBCONTRACT_ID+"&contractType="+contractType+"&applyId="+applyId+"&view="+v+"&conSubconId="+row.CON_SUBCON_ID+"&applyId="+applyId,
	            title: "担保合同信息", 
	            width: 800,
	            height: 500,
	            state:"max",
    ...
    function view(){
        ...
        if(row){
	        nui.open({
	            url: nui.context + "/crt/con_grt/con_dy_tab.jsp?subcontractId="+row.SUBCONTRACT_ID+"&contractType="+contractType+"&applyId="+applyId,
	            title: "担保合同信息", 
	            width: 800,
	    		height: 500,
	    		state:"max",
