...
<div id="layout" class="nui-layout" style="width:100%;height:100%;">
	<div title="出账放款" region="west"  width="240" class="sub-sidebar" allowResize="true">
...
<script type="text/javascript">
	...
	var processStatus = "<%=request.getParameter("processStatus")%>";
		...
		if(!e.node['url']) {
		 ...
		 if (e.node['id']=='意见') {
			...
			if(ret == "1"){
	 		...
	 		if(!CrdUtil.checkLimit("com.bos.crd.LimitService.riskLimitValidByLoanId.biz.ext",{"loanId":loanId})){
	 			return ;
	 		}
		} 	
	if (e.node['id']=='ecm') {
	debugger;
		    ...
		    $.ajax({
		        ...
		        success: function (mydata) {
				    ...
				    debugger;
		        	//跳往信息平台页面
						var ecmurl =nui.context + '/pub/imagePlatform/item_tree.jsp?businessNumber='+o.contactInfo.contractNum+'&csmNum='+o.party.partyNum+'&partyTypeCd=01&ismove='+ifDataMove+"&view=2&flowModuleType="+flowModuleType+'&postCode='+postCode;
					if(proFlag=="0"||proFlag=="-1"){
						var ecmurl =nui.context + '/pub/imagePlatform/item_tree.jsp?businessNumber='+o.contactInfo.contractNum+'&csmNum='+o.party.partyNum+'&partyTypeCd=01&ismove='+ifDataMove+"&view=1&flowModuleType="+flowModuleType+'&postCode='+postCode;
					}
		        	nui.open({
		                url:ecmurl,
		                title: "查看影像信息", 
