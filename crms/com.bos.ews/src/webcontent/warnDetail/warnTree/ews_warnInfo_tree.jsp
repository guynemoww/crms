...
<div id="layout1" class="nui-layout" style="width:100%;height:100%;">
	<div title="客户预警" region="west" width="240" class="sub-sidebar" allowResize="false">
...
<script type="text/javascript">
	nui.parse();
	var operateType = "<%=request.getParameter("operateType") %>";//操作类型
	var bizId = "<%=request.getParameter("bizId") %>";
	var partyId = "<%=request.getParameter("corpid") %>";
	var levelEdit = "<%=request.getParameter("isLevelEdit") %>";
            	...
            	if(operateType == "add"){
	            	menudata = [
	                     {id:"客户预警", text:"客户预警", //expanded:true, 
	                       children:[
                                {id:"新增预警信号",text:"新增预警信号",url:"/ews/warnDetail/warnInfo/ews_warnInfo_add.jsp?party="+nui.encode(party)+"&bizId="+bizId+"&type="+party.partyTypeCd+"&corpid="+party.partyId+"&processInstId="+processInstId+"&proFlag="+proFlag+"&flowType=1&node="+nui.encode(node)},
                                {id:"预警报告",text:"预警报告",url:"/ews/warnDetail/warnInfo/relevantFile.jsp?party="+nui.encode(party)+"&bizId="+bizId+"&proFlag="+proFlag}
                                ]}
	                       ];  
            	}else{
            		menudata = [ 
            		      {id:"客户预警", text:"客户预警",
                        	children:[
                              {id:"预警信号关闭",text:"预警信号关闭",url:"/ews/warnDetail/warnInfo/ews_warnInfo_close.jsp?bizId="+bizId+"&proFlag="+proFlag+"&corpid="+party.partyId+"&processInstId="+processInstId+"&flowType=2"},
