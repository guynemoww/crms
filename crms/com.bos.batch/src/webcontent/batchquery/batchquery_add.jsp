...
<div id="form1" style="width:94%;height:auto;overflow:hidden;text-align:left">
	...
	<div class="nui-dynpanel" columns="4">
		<label>批量代码：</label>
		<input name="acBatch.batchCode" required="true" class="nui-textbox nui-form-input" vtype="maxLength:80" />
		<label>执行时是否判断：</label>
		...
		<label>批量名称：</label>
		<input name="acBatch.batchName" required="true" class="nui-textbox nui-form-input" vtype="maxLength:80" />
		<label>批量错误是否后续影响：</label>
		<input name="acBatch.batchErrkeep" required="true" class="nui-dictradiogroup nui-form-input" vtype="maxLength:4"
		data="data" valueField="dictID" textField="dictName" dictTypeId="XD_RZCD0005" value="1"/>
		<label>批量启用状态：</label>
		<input name="acBatch.batchStatus" required="true" class="nui-dictcombobox nui-form-input" vtype="maxLength:4"
		...
		data="data" valueField="dictID" textField="dictName" dictTypeId="XD_RZCD0007" value="1" enabled="false" id="batchType" />
		<label>批量执行逻辑名：</label>
...
function save() {
	...
	$.ajax({
        url: "com.bos.batch.batchquery.addAcBatch.biz.ext",
        type: 'POST',
        data: json,
        cache: false,
