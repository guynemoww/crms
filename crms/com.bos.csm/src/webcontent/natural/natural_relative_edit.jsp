...
<input name="item.custType" id="item.custType" class="nui-hidden nui-form-input" />
		<div class="nui-dynpanel" columns="4">
			...
			<label>证件类型：</label>
			<input id="item.certificateTypeCd" required="true"  Enabled="false" name="item.certificateTypeCd" class="nui-dictcombobox nui-form-input"  dictTypeId="XD_ZJLX0001" />
	...
	var qote = "<%=request.getParameter("qote")%>" ;
		...
		function initForm(){
			...
			$.ajax({
				...
				success : function(mydata) {
					...
					nui.get("item.appellation").setText(nui.getDictText("XD_GLGX0001",o.item.appellation));
						//初始化证件到期日
		...
		function save() {
           form.validate();
			...
			var certNum1=nui.get("item.certId").getValue();
 			if(appellation=="1120"&&certNum!=certNum1){//如果关联关系选择配偶，校验唯一性
