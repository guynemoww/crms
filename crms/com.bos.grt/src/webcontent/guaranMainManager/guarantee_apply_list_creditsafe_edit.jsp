...
<head>
	...
	<title>编辑业务申请下的应收账款买断项下的信用保险</title>
</head>
<body>
	<div id="form1" style="width:100%;height:auto;overflow:hidden;">
		...
		<input name="tbGrtSurety.suretyId" required="true" class="nui-hidden" id="tbGrtSurety.suretyId" enabled="false"
			value="<%=request.getParameter("suretyId") %>" />
		...
		<div class="nui-dynpanel" columns="4">
			...
			<label>最低赔偿比例(%)：</label>
			<input name="tbGrtCreditsafe.minLossRate" required="true" class="nui-textbox" vtype="float;range:0,100;maxLength:6;" />
	...
	<div class="nui-toolbar" style="border-bottom:0;text-align:center;">
		<a class="nui-button" id="btnSave" iconCls="icon-save" onclick="save()" id="btnSave">保存</a>
	</div>
	...
	<script type="text/javascript">
		 ...
		 */
		function initForm() {
			var json=nui.encode({"tbGrtCreditsafe":{"suretyId":"<%=request.getParameter("suretyId") %>"}});
			var json2=nui.encode({"tbGrtGuaranteeBasic":{"suretyId":"<%=request.getParameter("suretyId") %>"}});
			var json3=nui.encode({"tbBizRelation":{"relationId":"<%=request.getParameter("relationId") %>","bizCustType":"<%=request.getParameter("bizCustType") %>",
				"applyId":"<%=request.getParameter("applyId") %>"}});
			var json4=nui.encode({"tbGrtSurety":{"suretyId":"<%=request.getParameter("suretyId") %>"}});
			var json5 = json + json2+ json3 + json4;
			git.mask();
			$.ajax({
