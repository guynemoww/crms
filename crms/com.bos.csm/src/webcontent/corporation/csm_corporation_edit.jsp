...
<!-- 			 <label>经营场地面积(平方米)：</label>
		...
		<input id="item.owner"           name="item.owner"           required="true"  class="nui-dictcombobox nui-form-input" dictTypeId="CDJYSYQ00"/> -->
    		...
    		<label>组织机构代码：</label>
			<input id="item.orgRegisterCd" name="item.orgRegisterCd" required="true" enabled="false"
			...
			<label>机构信用代码：</label>
			<input id="item.orgCreditCode" name="item.orgCreditCode" required="false" vtype="minLength:18;maxLength:18"
				class="nui-textbox nui-form-input" />
    ...
    <fieldset>
  	<legend>
    	...
    	<label>行业信息参考：</label>/csm/corporation/csm_corporation_edit.jsp
    	<a href="<%=request.getContextPath()%>/csm/corporation/doc/industrialType.pdf" target="_blank">国民经济行业分类注释</a>
    	</div>
    </fieldset>
...
<script type="text/javascript">
	...
	function checkTaxNo(e){
		if(e.value.length>0){
			CsmValidateobj.onValidateTax(e);
			return ;
		}
	}
    ...
    $(document).ready(function(){
    			   	  var arr = git.getDictDataFilter("XD_KHCD0252","1,9");
    	...
    	unifySocietyCreditNumEdit=text.item.unifySocietyCreditNum;
		         //修改注册资本倍数
	            if(text.item.registerAssets && text.item.registerAssets>0){
	            	text.item.registerAssets = text.item.registerAssets/10000;
	            }
	            
               	form.setData(text);//text.map
               	//初始化成立日期
                ...
                if(text.map.party.isPotentialCust=='1'){//授信客户不能修改为非授信
                	nui.get('party.isPotentialCust').setEnabled(false);
                	if(text.map.item.areaType=="1"){//境内
                	    nui.get("item.middelCode").setRequired(true);
                	}
                }
                if(text.map.party.examineState=='3'){//通过完整性校验的客户不能点击临时保存
	                nui.get('btnTest').setEnabled(false);
	            }
	            if(text.map.item.unifySocietyCreditNum!=null&&text.map.item.unifySocietyCreditNum!=""){//如已录入“统一社会信用代码”，则国税、地税登记证号码非必输
	 ...
	 function validRegisterAssets(e){
		debugger;
	 	if (e.isValid) {
	 		var xx = Number(e.value);
	        if (xx==null || isNaN(xx) || ( xx !=0 && xx < 1 )) {
	            e.errorText = "该户注册资本金额小于1万元人民币，请核实";
	            e.isValid = false;
	        }
	    }
	} 
	...
	function dataCheck(){
		...
		var isPotentialCust = nui.get("party.isPotentialCust").getValue();//是否信贷客户
...
/* 		var middleCode=nui.get("item.middelCode").getValue();
		...
		debugger;
		//修改注册资本倍数
        if(o.item.registerAssets && o.item.registerAssets>0){
          var registerAssets=o.item.registerAssets*10000;
          registerAssets=registerAssets.toFixed(2);
        	o.item.registerAssets = registerAssets;
        }
