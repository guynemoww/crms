...
<script type="text/javascript">
	nui.parse();
		...
		var partyId = "<%=request.getParameter("corpid") %>";  //获取预警情况变更ID
	    var countrys=[{ "id": "01", "text": "红色一级预警" },{ "id": "02", "text": "红色二级预警" },{ "id": "03", "text": "橙色一级预警" },
	                  { "id": "04", "text": "橙色二级预警" },{ "id": "05", "text": "黄色一级预警" },{ "id": "06", "text": "黄色二级预警" },
...
function initForm() {
      ...
      // var entity=nui.get("entity").value;
       ...
       $.ajax({
            url: "com.bos.ews.warnMgr.getCsmWarnLevelInfo.biz.ext",
            type: 'POST',
            data: json,
            cache: false,
...
function update(){
    ...
    var json=nui.encode({levelInfo:levelInfo,partyId:partyId});
   $.ajax({       
            url: "com.bos.ews.warnMgr.editCorpLevel.biz.ext",
            type: 'POST',
            data: json,
            cache: false,
            contentType:'text/json',
            success: function (text) {
                 //alert(text.msg);
                	           ...
                	           openSubmitView(node);
                               //grid.load({partyId:corpid,status:"1"});
                               //form3.reset();
                               //initForm();//初始化
               	               git.unmask();
