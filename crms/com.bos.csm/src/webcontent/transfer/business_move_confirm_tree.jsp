...
-->
<head>
<title>业务移交审批意见</title>
<%@include file="/common/nui/common.jsp"%>
</head>
<body>

	<div id="layout1" class="nui-layout" style="width: 100%; height: 100%;">
		<div title="业务移交审批意见" region="west" bodyStyle="" width="240"
	...
	function initForm() {
		if(wflow=='1'){
			menudata = [
				{id:"移交信息",text:"移交信息",url:"/csm/transfer/business_move_confirm.jsp?bizId=<%=request.getParameter("bizId")%>&processInstId=<%=request.getParameter("processInstId")%>&isSrc=<%=request.getParameter("isSrc")%>"}
			];
		}else{
			menudata = [
				{id:"移交信息",text:"移交信息",url:"/csm/transfer/business_move_confirm.jsp?bizId=<%=request.getParameter("bizId")%>&processInstId=<%=request.getParameter("processInstId")%>&isSrc=<%=request.getParameter("isSrc")%>"},
