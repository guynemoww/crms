...
<body style="margin-left:10px;">
 <div id="check">
	...
	<div class="nui-dynpanel" columns="2">
		<label>当前检查频率：</label>
		<input id="InspectRate" name="setRate" class="nui-text nui-form-input"  dictTypeId="XD_DHCD0001"/>
    </div>
    </fieldset>
    ...
    <fieldset style="margin-top: 10px;">
		...
		<div colspan="3">
		    <div id="valuation" class="nui-dynpanel" columns="4">
		         ...
		         <div>
		            <input name="value1" class="nui-textbox nui-form-input" dataType="float" style="width:80%;">万元</input>
		         </div>
		
		         <div style="text-align: center;">其他：</div>
		         <div>   
		            <input name="value2" class="nui-textbox nui-form-input" dataType="float" style="width:80%;">万元</input>
		         </div>
	        </div>
	    </div>
	</fieldset>
...
<fieldset>
	...
	<div id="datagrid1" class="nui-datagrid" showPager="false" url="com.bos.aft.aft_inspect_report.getInspectCond.biz.ext" dataField="inspectConds" idField="id" allowCellEdit="true" allowCellSelect="true"  editNextOnEnterKey="true" >
...
<fieldset style="margin-top: 10px;">
	...
	<div class="nui-dynpanel" columns="6" textAlign="left">
	    ...
	    <input id="lastYearFinancingBalance" name="lastYearFinancingBalance" dataType="float" class="nui-textbox nui-form-input" style="width:100%;"> 
		<div style="text-align: left;width: 100%;">上期检查时融资余额</div>
		<input id="lastFinancingBalance"  name="lastFinancingBalance" dataType="float" class="nui-textbox nui-form-input" style="width:100%;"> 
		<div style="text-align: left;width: 100%;">本期检查时融资余额</div>
		<input id="currentFinancingBalance" name="currentFinancingBalance" dataType="float" class="nui-textbox nui-form-input" style="width:100%;"> 
    </div>     
</fieldset>	   
 ...
 <div id="form">
	...
	<div>
	 <input id="realEstateLoanExplain2" name="realEstateLoanExplain2" class="nui-textarea nui-form-input"  style="width:100%;"/>
	</div>
	<label><span >销售情况具体描述：（包括但不限于销售成交量、成交价、按计划应达到的成交量、成交价，实际销售情况是否与计划一致，不一致的具体原因及拟采取的相应措施等）:</span></label>
	<div>
	 <input id="realEstateLoanExplain3" name="realEstateLoanExplain3" class="nui-textarea nui-form-input"  style="width:100%;"/>
	</div>
...
<div id="warn">
	...
	<fieldset style="margin-top: 10px;margin-bottom: 10px;">
		   ...
		   <div style="border-bottom: 1px"> 
		     ...
		     <div>
		        <input name="executionPlan" class="nui-textarea" style="width:100%;" required="false"/>
		     </div>
	</fieldset>	  
	</div>
</div>
...
<fieldset style="margin-top: 10px;margin-bottom: 10px;">
 ...
 <div id="zeren" style="width:99.5%;height:auto;overflow:hidden;margin-top: 10px;">
...
<script type="text/javascript">
    ...
    nui.parse();
     ...
     var editform = new nui.Form("#editform");                                            //检查方式
     var form = new nui.Form("#form");                                                    //相关指标部分      
     var valuation = new nui.Form("#valuation");                                          //获取估值信息
     var captial = new nui.Form("#captial");                                              //资金流向部分
     var inspectBiz ;                                                                     //贷后检查报告
	 ...
	 var bizId = "<%=request.getParameter("bizId") %>";                                   //贷后检查报告业务ID
	 var partyId ;                                                                        //参与人ID
	 var inspectRate ;                                                                    //检查频率
	//选择岗位人员
	...
	function getUserName(){
	 	 $.ajax({
	            ...
	            success: function (text) {
	            	 nui.get("postName3").setText(text.userNames);
	            	 nui.get("postName3").setValue(text.userNames);
	            }
	        });
    ...
    //加载时根据值判断显示文本框
    function checkJudge(){ 
	    var wayCd = inspectBiz.inspectWayCd;
	    //alert(inspect.inspectWayCd);
	    //alert(wayCd);  RECEPTION_TYPE_CD 
	    var html1 = "<div columns='4' class='nui-dynpanel' id='inspectform'><label >检查地点</label> <input colspan='3' name='inspectPlace' class='nui-tex nui-form-input' required='true'/></div>";
        var html2 = "<div columns='4' class='nui-dynpanel' id='inspectform'><label>人员类型</label> <input name='receptionTypeCd' class='nui-text nui-form-input' dictTypeId='XD_DHCD0010' colspan='2' required='true' enabled='false'/></div>";
        var html3 = "<div columns='4' class='nui-dynpanel' id='inspectform'><label colspan='2'>其他检查方式</label> <input colspan='4' name='otherInspectWay' class='nui-text nui-form-input' required='true' style='width:100%;'/></div>";
	    var html4 = "";
	    if(wayCd==1){
	...
	function getInspectBiz(){
	 var json = nui.encode({bizId:bizId});
	    $.ajax({
            ...
            async:false,
            success: function (text) {
            	inspectBiz= text.inspect;
            	partyId = inspectBiz.partyId;
            	inspectRate = text.inspectRate.setRate;
            	//alert(nui.encode(inspectBiz));
            },
    ...
    function init(){
       getInspectBiz();
       var wholeEvaluationChoose =  inspectBiz.wholeEvaluationChoose;               //获取整体评价选项
       
                                       //拆分组合的整体评价
       form.setData(inspectBiz);                                                    //指标项加载数据
       nui.get("inspectWayCd").setValue(inspectBiz.inspectWayCd);
       check.setData(inspectBiz);                                                   //贷后检查基本信息数据
       warn.setData(inspectBiz);                                                    //预警部分信息
       var irId = inspectBiz.irId;
       grid.load({bizId:irId});
       overAllEvaluation.setData(inspectBiz);                                       //整体评价部分信息
       getUserName();
       //valuation.setData(text.valuation);
      // alert("wholeEvaluationChoose:"+wholeEvaluationChoose);
       if(typeof(wholeEvaluationChoose) != "undefined" && wholeEvaluationChoose!="0000"){
         //alert(111);
         var reslt = wholeEvaluationChoose.split(",");
       ...
       nui.get("lastYearFinancingBalance").setValue(inspectBiz.lastYearFinancingBalance);   //上年他行融资余额
       nui.get("lastFinancingBalance").setValue(inspectBiz.lastFinancingBalance);           //上期他行融资余额
       nui.get("currentFinancingBalance").setValue(inspectBiz.currentFinancingBalance);     //本期他行融资余额
...
function create(e,n,l) {
		...
		var pageJsonData=nui.encode({"indexCd":pageName,lastIndicators:lastIndicators,idName:idName,inspect:inspectBiz});
		var data1s;
		var data2s;
		$.ajax({
		   ...
		   temp.children = options;
		    	 //alert("data1s:"+nui.encode(data1s));
		   if(null == data1s || null == data1s.indexChoice || data1s.indexChoice == ''|| data1s == null) {//加载完页面，保存初始化选项的值（包括首次加载页面和之后的每次修改）
		   	   temp.data1 = '';//没有选项
		   ...
		   //initChoices[0] = temp.data1;//初始化的选项值
		    ...
		    //上期选择
		    if(data2s == null || data2s == null || data2s.comment == null ) {
		        temp.data2 = "";
		    } else {
		    ...
		    if(data1s == null 
		     ...
		     //alert("value:"+nui.encode(nui.get(idName)));
		    // alert("temp:"+nui.encode(temp.data1)+"   idName:"+idName);
...
function setOtherInfo(){
	...
	//增加一行
	function btnAdd(){		        var index=grid.totalCount;
    ...
    function save(){
       ...
       for(var i=0;i<name.length;i++){                          //取得相关指标
					   ...
					   switch (idName){ 
                              ...
                              case name[1]:inspect["borrowFinanceChoose"] = data.data1;
                                           inspect["borrowFinanExplain1"] = data.data3;
                                           inspect["borrowFinanExplain2"] = nui.get("borrowFinanceExplain2").getValue(); 
                                         break;
                              ...
                              case name[3]:inspect["realEstateLoanChoose"] = data.data1;
                                           ...
                                           inspect["realEstateLoanExplain3"] = nui.get("realEstateLoanExplain2").getValue();  
     ...
     var checkData = nui.decode(check.getData());
     var wayCd = nui.get("inspectWayCd").getValue();
	    if(wayCd==1){   
     ...
     */
     ...
     var json = nui.encode({inspect:nui.decode(inspect),inspectBiz:inspectBiz,inspectCon:inspectCon});
      $.ajax({
