...
<body>
	<div id="form1" style="width:100%;height:auto;overflow:hidden;">
		<input type="hidden" name="outcoolinfo" class="nui-hidden" />
	</div>
	<div id="grid1" class="nui-datagrid" style="width:100%;height:90%" 
		url="com.bos.grt.regmanage.collateralout.queryOutCoolinfoList.biz.ext"
		dataField="outcoolinfos"  allowResize="false" showReloadButton="false" 
	...
	<%if(request.getParameter("activityDefId")!=null && (request.getParameter("activityDefId").equals("P1001")||request.getParameter("activityDefId").equals("P2001")||request.getParameter("activityDefId").equals("P3001")||
		...
		<div class="nui-toolbar" style="border-bottom:0;text-align:center;">
			<a class="nui-button" iconCls="icon-ok" onclick="ChuKuCancel()">撤销</a>
		</div>
	<%}%>
	...
	<script type="text/javascript">
	    ...
	    function search(){
	      	var data  = {"outcoolinfo":{"stockRemovalId":bizId}};
	...
	//出库撤销
	function ChuKuCancel(){
	   var stockRemovalId = "<%=request.getParameter("bizId")%>";
		nui.confirm("确定出库撤销吗？","确认",function(action){
			if(action!="ok") return;
			var json=nui.encode({"stockRemovalId":stockRemovalId});
			$.ajax({
				url: "com.bos.grt.regmanage.collateralout.cancelChuKuFlow.biz.ext",
				type: 'POST',
				data: json,
				cache: false,
