...
<div id="form1" style="width:99.5%;height:auto;overflow:hidden;">
	...
	<input name="tbBizTxxxApply.loanid" id="tbBizTxxxApply.loanid" class="nui-hidden" />
	<fieldset>
 		<legend>
  		<span>纸票信息</span>
  		</legend>
  		<div class="nui-dynpanel" columns="4" id="div_1">
			...
			<label>汇票号码 :</label>
			<input id="tbBizTxxxApply.billno" name="tbBizTxxxApply.billno" required="true" class="nui-text" vtype="maxLength:100"  />
			<label>利率(%):</label>
		...
		<div class="nui-dynpanel" columns="4" id="div_2">
			...
			<label>出票日期:</label>
			<input id="tbBizTxxxApply.billbegindate" name="tbBizTxxxApply.billbegindate" required="true" class="nui-datepicker nui-form-input" allowInput="true"/>
			<label>到期日期:</label>
			<input id="tbBizTxxxApply.billenddate" name="tbBizTxxxApply.billenddate" required="true" class="nui-datepicker nui-form-input" allowInput="true"/>
		</div>
		<span> </span>
		<div class="nui-dynpanel" columns="4" id="div_2">
			<label>出票人全称:</label>
...
<script type="text/javascript">
	...
	function initForm() {
		...
		$.ajax({
			url: "com.bos.bizProductDetail.bizPjxx.GetTxxx.biz.ext",
			type: 'POST',
			data: json,
			cache: false,
			...
			success: function (text) {
				git.unmask("form1");
				form.setData(text);
				nui.get("tbBizTxxxApply.takeoutacbankname").setText(text.tbBizTxxxApply.takeoutacbankname);
				nui.get("tbBizTxxxApply.benebankname").setText(text.tbBizTxxxApply.benebankname);
				nui.get("tbBizTxxxApply.billbankname").setText(text.tbBizTxxxApply.billbankname);
				if(text.tbBizTxxxApply.interate){
					nui.get("tbBizTxxxApply.interate").setEnabled(false);
				}
				// 银行承兑汇票 承兑人不可编辑 账号为0 名称联动承兑行
				if(text.tbBizTxxxApply.billtype == "0"){
					nui.get("tbBizTxxxApply.billacname").setEnabled(false);
					nui.get("tbBizTxxxApply.billacname").setValue(text.tbBizTxxxApply.billbankname);
					nui.get("tbBizTxxxApply.billacno").setEnabled(false);
					nui.get("tbBizTxxxApply.billacno").setValue("0");
				}
				if(text.tbBizTxxxApply.billno && text.tbBizTxxxApply.billno.length>26){
					document.getElementById("tbBizTxxxApply.billno").style.width="95%";
				}
			},
	...
	function save() {
		...
		$.ajax({
			url: "com.bos.bizProductDetail.bizPjxx.UpdateTxxx.biz.ext",
			type: 'POST',
			data: json,
			cache: false,
			contentType:'text/json',
